...
var _displayed_reward_id: String
var _displayed_condition: UnlockCondition
var _displayed_save_game_state: SaveGameState
...
func _refresh_text() -> void:
	### For localization
	if not _displayed_condition:
		return
	display_condition_status(_displayed_reward_id, _displayed_condition, _displayed_save_game_state)

func display_condition_status(reward_id: String, condition: UnlockCondition,save_game_state: SaveGameState) -> void:
	if not condition:
		return
	_displayed_reward_id = reward_id
	_displayed_condition = condition
	_displayed_save_game_state = save_game_state
	...
	match condition_type:
		...
		Condition_IsEncounterCleared:
			var encounter_name: String = AutoloadDatabase.encounter_definitions[condition.encounter_id].get_encounter_name()
		...
		Condition_SimultaneousCumInGivenEncounter:
			...
			var encounter_name: String = encounter_def.get_encounter_name()
		...
		Condition_DontPlayGivenCardsInEncounter:
			var encounter_id: String = condition.encounter_id
			var encounter_def: EncounterDefinition = AutoloadDatabase.get_encounter_def_by_id(encounter_id)
			var encounter_name: String = encounter_def.get_encounter_name()
			var templated_string: String = tr("UI_UNLOCK_CONDITION_DONT_PLAY_CARDS_TEMPLATE")%encounter_name
			var cards_to_not_play: Array[String] = condition.cards_to_not_play
			var cards_to_avoid_string: String = ""
			var count: int = 1
			for card_id in cards_to_not_play:
				var card_def: EventCardDefinition = AutoloadDatabase.get_event_card_def_by_id(card_id)
				var card_name: String = card_def.get_card_name()
		
				var card_string: String = "[b][i]"+card_name+ "[/i][/b]"
				if count < cards_to_not_play.size():
					card_string +=", "
				count += 1
				cards_to_avoid_string += card_string
			
			description.text = templated_string + cards_to_avoid_string

...
### GDE GoZen (the video player as of this branch) decodes via FFmpeg, so modders are no
### longer limited to .ogv - kept .ogv for backward compatibility with existing mods, added
### the containers confirmed present in the compiled GoZen library (checked its bundled
### FFmpeg build directly) and commonly used for game clips. Codec support within a
### container isn't validated here - an unsupported codec inside an accepted container will
### surface as a load failure at actual playback time, not at import time. Keep in sync with
### VideoClip.video_file's @export_file filter (video_clip.gd) - that one has to be a
### compile-time constant string, so it can't just reference this array.
const SUPPORTED_MOD_VIDEO_EXTENSIONS: Array[String] = [".ogv", ".mp4", ".m4v", ".mov", ".mkv", ".webm", ".avi"]

var database: VideoDatabase

var _imported_video_file_names: Dictionary = {} ### sanitized file name -> true, tracks dupes across all mods this load

func _ready() -> void:
	### AutoloadDatabase's content-pack scan now runs in the background (see its
	### _ready()) - wait for it before reading video_clips from it, or this would
	### permanently build the database with zero built-in clips.
	AutoloadDatabase.run_when_content_loaded(_finish_loading_video_database)

func _finish_loading_video_database() -> void:
	database = _load_video_clips_into_database()

func _load_video_clips_into_database() -> VideoDatabase:
	var new_database: VideoDatabase = VideoDatabase.new()
	### Built-in clips are collected by AutoloadDatabase's own content_packs/ scan
	### (AutoloadDatabase must run before this autoload - see project.godot order) so
	### this doesn't redundantly re-scan and re-load every .tres in content_packs/.
	new_database.video_clips.append_array(AutoloadDatabase.video_clips)
	var clips_from_mods: Array[VideoClip] = _load_mod_video_clips()
...
### Claude-induced modding, added 19.07.26 — unchanged, this is a separate system (user:// JSON mods)
...
func _build_video_clip_from_json(entry: Dictionary, videos_path: String) -> VideoClip:
	...
	if not _has_supported_video_extension(sanitized_file_name):
		push_warning("Rejected clip '%s': supported formats are %s." % [
			sanitized_file_name, ", ".join(SUPPORTED_MOD_VIDEO_EXTENSIONS)])
		return null
...
func _has_supported_video_extension(file_name: String) -> bool:
	var lower_name: String = file_name.to_lower()
	for extension in SUPPORTED_MOD_VIDEO_EXTENSIONS:

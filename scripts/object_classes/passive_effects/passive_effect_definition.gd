...
### Generic description support (see DescriptionBuilder) - used when something (e.g.
### TargetsOpponentsWithPassive) wants to nest-reference this passive, same role as
### StatusEffectDefinition.get_description_segments(). Reuses the full Modifiers/Triggers
### tooltip (get_tooltip_description_segments()) rather than a separate cut-down summary, since
### "hover this passive to see what it does" and "the standalone passive tooltip" want the same
### content; falls back to the hand-written passive_description if it has neither.
...
func get_combo_progress(game_state: GameState) -> Dictionary:
	...
	if not combo_effect:
		return {}
	var current: int = game_state.encounter_report.get_times_cards_have_been_played(combo_effect.card_ids)
...
### Hand-written rather than the generic ModExportable default because picture: Texture2D is a
### file reference, not embeddable data - see get_file_reference_fields() above. The nested
### modifier/tick/triggered components still serialize via their own (generic) to_json_dict().
func to_json_dict() -> Dictionary:
	...
	return {
		...
		"has_secret_description": has_secret_description,
...
### mod_folder_path resolves picture (see get_file_reference_fields()); component reconstruction
### goes through ModdableResourceRegistry.instantiate() since each component dict carries its own
### "_class" tag from the generic serializer.
static func from_json_dict(data: Dictionary, mod_folder_path: String = "") -> PassiveEffectDefinition:
	...
	passive_def.has_secret_description = data.get("has_secret_description", false)

	var picture_file_name: String = data.get("picture", "")
	if picture_file_name != "" and mod_folder_path != "":
		passive_def.picture = Utils.load_texture_from_path(mod_folder_path.path_join(picture_file_name))

	for component_data in data.get("modifier_components", []):
		var component: StatusModifierComponent = ModdableResourceRegistry.instantiate(component_data) as StatusModifierComponent
		if component:
			passive_def.modifier_components.append(component)
	for component_data in data.get("ticking_effect_components", []):
		var tick_component: StatusTickComponent = ModdableResourceRegistry.instantiate(component_data) as StatusTickComponent
		if tick_component:
			passive_def.ticking_effect_components.append(tick_component)
	for component_data in data.get("triggered_components", []):
		var triggered_component: StatusTriggeredComponent = ModdableResourceRegistry.instantiate(component_data) as StatusTriggeredComponent
